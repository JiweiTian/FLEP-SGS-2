...
try 
{
    ...
    if (argc == 3) {
        cout << "State estimation of IEEE " << argv[1] << "-bus system" << " with bus " << argv[2] << " in the HITL" << endl;
        hitlflag = true;   
    } 
    else if (argc == 2) {
        cout << "State estimation of IEEE " << argv[1] << "-bus system" << endl;
        hitlflag = false;
    }
    else if (argc < 2) {
        cerr << "Usage: " << argv[0] << " <number of IEEE bus system>" << " <bus of HITL><optional> " << endl;
        return 1;
    }
    ...
    */
    ...
    for(int runs=0; runs<1000000; runs++) {
        ...
        zdatas(N,zd,out,bus,G,B,bbus,hitlflag);
        for(i=0; i<2; i++){
            out[i].clear();
        }
